...
def get_arguments():
    ...
    parser.add_argument("--fnac_loss3_weight", type=float, default=100, )
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Dataloaders
    # if args.trainset in ['flickr_10k', 'flickr_144k']:
    traindataset = get_train_dataset(args)
    # else:
        # traindataset = get_train_dataset_2(args)
    train_sampler = None
    if args.multiprocessing_distributed:
    ...
    # testdataset = get_test_dataset_2(args)
    # test_loader = torch.utils.data.DataLoader(
    #     testdataset, batch_size=args.batch_size, shuffle=(train_sampler is None),
    #     num_workers=args.workers, pin_memory=False, sampler=train_sampler, drop_last=True,
    #     persistent_workers=args.workers > 0)
...
def train(train_loader, model, optimizer, epoch, args):
    ...
    for i, (image, spec, _, file_id) in enumerate(train_loader):
        ...
        if epoch < args.warmup:
            ...
            loss = loss[0] + args.fnac_loss1_weight*loss[1] + args.fnac_loss2_weight*loss[2]  + args.fnac_loss3_weight * loss[3] 
